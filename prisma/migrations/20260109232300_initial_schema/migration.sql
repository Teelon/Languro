...
-- Function: Ensure English infinitives always have "to" prefix
CREATE OR REPLACE FUNCTION enforce_english_infinitive_format()
RETURNS TRIGGER AS $$
BEGIN
    -- Trim whitespace for all languages
    NEW.word = TRIM(NEW.word);
    
    -- Only apply to English verbs (iso_code = 'en')
    IF EXISTS (
        SELECT 1 FROM languages 
        WHERE id = NEW.language_id 
        AND iso_code = 'en'
    ) THEN
        -- If the word doesn't start with "to ", add it
        IF NEW.word !~ '^to\s+' THEN
            NEW.word = 'to ' || TRIM(NEW.word);
        END IF;
    END IF;
...
-- Trigger: Apply format enforcement on insert/update
CREATE TRIGGER tr_enforce_english_infinitive
BEFORE INSERT OR UPDATE ON verb_translations
FOR EACH ROW
EXECUTE FUNCTION enforce_english_infinitive_format();
...
-- Index for fuzzy search on verb infinitives
CREATE INDEX idx_verb_translations_word_trgm 
ON verb_translations 
USING gin (word gin_trgm_ops);

-- Index for fuzzy search on conjugated forms
CREATE INDEX idx_conjugations_display_trgm 
ON conjugations 
USING gin (display_form gin_trgm_ops);
